//! The real browser IndexedDB Workshop store.
//!
//! Extracted verbatim from the `mod wasm` block of the parent module.
//! `super::` still resolves to `crate::workshop::store::web`, exactly as it
//! did inline, so no path in these bodies changed. Nothing in this file is
//! compiled by the host gate: `cargo check --target wasm32-unknown-unknown`
//! and the wasm clippy pass are the only commands that see it.
...
impl ReferenceState {
    ...
    fn validate(&self) -> Result<(), WorkshopStoreError> {
        if (!self.slots.is_empty() && !self.has_metadata) || self.slots.len() > MAX_WORKSHOP_SLOTS {
            return Err(WorkshopStoreError::IndexedDbSchemaMismatch);
        }
        for (id, slot) in &self.slots {
            let id =
                usize::try_from(id.0).map_err(|_| WorkshopStoreError::IndexedDbSchemaMismatch)?;
...
async fn execute(request: WorkshopStoreRequest) -> Result<WorkshopStoreResult, WorkshopStoreError> {
    match &request {
        WorkshopStoreRequest::CreateSlot { archive, .. }
        | WorkshopStoreRequest::CommitSlot { archive, .. }
        | WorkshopStoreRequest::PromoteRecoveredSlot { archive, .. } => validate_archive(archive)?,
...
async fn read_references(store: &IdbObjectStore) -> Result<ReferenceState, WorkshopStoreError> {
    let request = store
        .get_all_with_key_and_limit(
            &JsValue::UNDEFINED,
            u32::try_from(MAX_WORKSHOP_SLOTS + 2).expect("Workshop slot query limit fits in u32"),
        )
        .map_err(map_js_error)?;
    ReferenceState::decode(request_result(request).await?)
}

async fn read_pack_hashes(store: &IdbObjectStore) -> Result<Vec<CatalogHash>, WorkshopStoreError> {
    let value = request_result(
        store
...
fn put_json_sync<T: serde::Serialize>(
    ...
    let value =
        serde_json::to_string(value).map_err(|_| WorkshopStoreError::IndexedDbSchemaMismatch)?;
    store
...
async fn read_generation(
    ...
    let value = request_result(store.get(&JsValue::from_str(&key)).map_err(map_js_error)?).await?;
    if value.is_undefined() {
...
async fn await_open_request(request: IdbOpenDbRequest) -> Result<OpenDatabase, WorkshopStoreError> {
    let handlers = Rc::new(RefCell::new(EventHandlers::empty()));
    let promise = Promise::new(&mut |resolve: Function, reject: Function| {
        let success_request = request.clone();
        let success_resolve = resolve.clone();
        ...
        handlers.borrow_mut().blocked = Some(Closure::wrap(Box::new(move |_event: Event| {
            let _ = blocked_reject.call1(&JsValue::UNDEFINED, &JsValue::from_str(BLOCKED_SENTINEL));
        }) as Box<dyn FnMut(_)>));
        ...
        handlers.borrow_mut().upgrade = Some(Closure::wrap(Box::new(move |_event: Event| {
            ...
            if result.is_err() {
                let _ =
                    upgrade_reject.call1(&JsValue::UNDEFINED, &JsValue::from_str(SCHEMA_SENTINEL));
