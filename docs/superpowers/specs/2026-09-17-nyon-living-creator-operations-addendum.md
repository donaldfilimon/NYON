...
- **RT.2** [D rules L171] `1 <= batch_size <= 10`; `cadence_ticks == 10`; `source_reserve >= 20`; codes `route_batch`, `route_cadence`, `route_reserve`. Since 2026-09-17 `validate` enforces all three for every state (fields `route batch_size`, `route cadence`, `route source_reserve`); see K5.
...
- **K5. The state validator is weaker than the rules it cites.** Route checks CLOSED 2026-09-17: `validate` now enforces rules L171's `batch <= 10`, `cadence == 10`, `reserve >= 20` (`LIVING_ROUTE_*_V2`). Facility HP is unchecked (`DP#8`). Reservation capacity is unchecked (`DP#7`, `model.rs:1216-1229`). Civilization status is never checked against colony count, though rules L220 ties them. This addendum closes the creator path only (RT.2, CF.3, X10, X17). A decoded archive or genesis can still carry these states.
